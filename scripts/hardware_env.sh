...
_g1_hardware_setup() {
  local script_dir workspace_dir network_interface unitree_setup

  script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
  workspace_dir="$(cd -- "${script_dir}/.." && pwd)"
  network_interface="${1:-${G1_HARDWARE_NETWORK_INTERFACE:-}}"
  ...
  echo "G1 hardware ROS environment: distro=${ROS_DISTRO}, domain=${ROS_DOMAIN_ID}, rmw=${RMW_IMPLEMENTATION}, interface=${network_interface:-auto}"
}
