...
включены официальный URDF Unitree G1 29 DoF и только необходимые ему meshes.
...
Важно: это именно **виртуальный захват**. В официальном G1 29 DoF URDF ладони
не имеют управляемых пальцев, поэтому после фазы grasp Gazebo создаёт detachable
fixed joint с правым запястьем, а левая кисть синхронно удерживает вторую сторону
коробки визуально. Детектор — геометрический depth-кластер в известной зоне
...
захват на реального робота без отдельного восприятия, full-body/arm planning,
реального управления кистями и safety-проверки.
